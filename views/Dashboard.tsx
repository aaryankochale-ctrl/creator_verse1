    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 snap-x">
          {MOCK_FREELANCERS.map((f) => (
            <div key={f.id} className="min-w-[240px] bg-gray-900 border border-gray-800 rounded-2xl p-4 shadow-sm snap-start">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-400 font-medium">{f.skills?.[0]}</p>
                </div>
              </div>
              <p className="text-xs text-gray-500 line-clamp-2 mb-3">{f.bio}</p>
              <button className="w-full py-2 bg-gray-800 text-gray-300 text-xs font-bold rounded-lg hover:bg-indigo-900 hover:text-indigo-400 transition-colors">
                View Profile
              </button>
            </div>
          ))}
      ...
      <section>
        ...
        <div className="space-y-3">
          {MOCK_GIGS.map((gig) => (
            <div key={gig.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex items-center justify-between hover:border-indigo-800 transition-colors">
              <div>
                <h4 className="font-bold text-sm mb-1 text-white">{gig.title}</h4>
